...
class SimpleNeuralNetwork:
    ...
    def backward(self, error):
        ...
        self.weights += self.learning_rate * np.dot(self.inputs.T, d_output)
...
for i in range(len(X_or)):
    ...
    print(f"Input: {X_or[i]}, Output: {prediction[0][0]}, Expected: {y_or[i][0]}")
...
for i in range(len(X_or)):
    ...
    print(f"Input: {X_or[i]}, Output: {prediction[0][0]}, Expected: {y_nor[i][0]}")
